...
void ImportanceUpdatingAgent::calculateAtomWages(AtomSpace *a, const AgentSeq &agents)
{
    ...
    for (size_t n = 0; n < agents.size(); n++) {
        ...
        log->debug("Agent getTotalStimulus: %d", agents[n]->getTotalStimulus());
...
void ImportanceUpdatingAgent::adjustSTIFunds(AtomSpace* a)
{
    ...
    foreach(Handle handle, hs) {
        ...
        if (actualTax != 0) {
            ...
            log->fine("Handle [%d]: sti %d. Actual tax %d. after tax %d.", 
                      handle.value(), beforeTax, actualTax, afterTax);
        }
    }
...
//        if (attentionalFocusSize.recent > 0)
        if (attentionalFocusSize.recent >= 1)    
            STITransitionalAtomRent.update((AttentionValue::sti_t)ceil((float) STIAtomWage * (float) totalStimulusSinceReset.recent \
...
*/
    ...
    log->fine("STIAtomRent was %d, now %d. Focus size was %.2f. Wage is %d. Total stim was %.2f.", 
              oldSTIAtomRent, STIAtomRent, focusSize, STIAtomWage, totalStimulusSinceReset.recent);
...
void ImportanceUpdatingAgent::updateAtomSTI(AtomSpace* a, const AgentSeq &agents, Handle h)
{
    ...
    for (size_t n = 0; n < agents.size(); n++) {
        ...
        // Only perform further calculations if the stimulus is non-zero
        if (s != 0) {
            total_stim += s;
